...
int MergeSort(int* ini, int* fin,int conta=0) {
    ...
    std::cout << " Estado actual -- Izq " << conta_izq << " Estado actual -- Der " << conta_der << std::endl;
    std::cout << " DIVISION " << std::endl;
    PrintDivision(ini, fin);
    ...
    std::cout << " ME VOY CON UN CONTA DE " << conta << std::endl;
    return conta + conta_der + conta_izq;
}
int main()
{
    int arr[12] = { 4,8,10,21,5,12,11,3,7,6,9,1 };
    std::cout << MergeSort(arr, arr + 11);
}
